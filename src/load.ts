  ...
  for(const a in data) {
    ...
    for (const l in activity.Locations) {
      ...
      for (const t  in location.TimeSlots) {
        ...
        timeslot.Activity = activity as Activity; // workaround duplicate failing to load downstream
        timeslot.Location = location as Location; // workaround duplicate failing to load downstream
        await TimeSlot.save(timeslot)
          .catch(e => logger.error(e.message));
      };
    };
  };
